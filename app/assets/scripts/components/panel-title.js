...
const PanelTitle = React.createClass({
  ...
  render: function () {
    const { title, subtitle, visible } = this.props
    ...
    return (
      <header className='panel__header'>
        ...
        <div className='panel__meta-actions'>
          <button className='panel__button-more' title='Options'><span>Options</span></button>
        </div>
        <div className='panel__tab-nav'>
          <ul className='layers-menu' role='menu'>
            ...
            <li><a onClick={this._handleBaseToggle} href='#' title='View base layers' className={c('layers-menu-item', {'layers-menu-item--active': visible === 'base'})}>Base layers</a></li>
          </ul>
        </div>
      </header>
    )
